...
class BenchmarkrateValidationException implements Exception {
  final List<String> errors;

  BenchmarkrateValidationException(this.errors);

  @override
  String toString() => 'BenchmarkrateValidationException: ${errors.join("; ")}';
}
...
/// Benchmark Rate API client.
///
/// For more information, visit: https://apiverve.com/marketplace/benchmarkrate?utm_source=dart&utm_medium=readme
///
/// Parameters:
/// * [rate] (required) - The benchmark rate to retrieve
class BenchmarkrateClient {
  final String apiKey;
  ...
  static final Map<String, ValidationRule> _validationRules = <String, ValidationRule>{
    'rate': ValidationRule(type: 'string', required: true),
  };

  BenchmarkrateClient(this.apiKey, {
    this.baseUrl = 'https://api.apiverve.com/v1/benchmarkrate',
    http.Client? httpClient,
  }) : _httpClient = httpClient ?? http.Client();

  /// Validates parameters against defined rules.
  /// Throws [BenchmarkrateValidationException] if validation fails.
  void _validateParams(Map<String, dynamic> params) {
    ...
    if (errors.isNotEmpty) {
      throw BenchmarkrateValidationException(errors);
    }
  }

  /// Execute a request to the Benchmark Rate API.
  ///
  /// Parameters are validated before sending the request.
  Future<BenchmarkrateResponse> execute(Map<String, dynamic> params) async {
    // Validate parameters
    _validateParams(params);
    if (apiKey.isEmpty) {
      throw BenchmarkrateException('API key is required. Get your API key at: https://apiverve.com');
    }
    ...
    try {
      ...
      if (response.statusCode == 200) {
        ...
        return BenchmarkrateResponse.fromJson(json);
      } else if (response.statusCode == 401) {
        throw BenchmarkrateException('Invalid API key');
      } else if (response.statusCode == 404) {
        throw BenchmarkrateException('Resource not found');
      } else {
        throw BenchmarkrateException('API error: ${response.statusCode}');
      }
    } catch (e) {
      if (e is BenchmarkrateException) rethrow;
      throw BenchmarkrateException('Request failed: $e');
    }
  }
...
/// Exception thrown by the Benchmark Rate API client.
class BenchmarkrateException implements Exception {
  final String message;

  BenchmarkrateException(this.message);

  @override
  String toString() => 'BenchmarkrateException: $message';
}
